    ...
    <?php if (!array_key_exists('id_notifica', $notifica)) : ?>
      ...
      <footer class="text-end my-3 px-3">
        <a class="btn btn-secondary m-3 rifiuta" href="#" role="button">
          Rifiuta</a>
        <a class="btn btn-primary m-3 accetta" href="#" role="button" data-bs-toggle="modal" onclick="setIdBook(<?php echo $notifica['ID_Libro']; ?>)" data-bs-target="#exampleModal">
          Scambia</a>
      </footer>
      </article>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Libri di <?php echo $username; ?></h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="#" method="Post">
          <input type="hidden" id="select" name="selected_book" value="book_id">
          <input type="hidden" id="select2" name="selected_book2" value="<?php echo  $notifica["ID_Libro"]; ?>">
          <?php $books = $dbh->getPostTable()->getPostLibroProfilo($username); // aggiorna la variabile $books con i libri dell'autore della notifica ?>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary conferma" data-bs-dismiss="modal" onclick="sendForm()">Conferma</button>
        </form>
      </div>
    </div>
  </div>
...
<?php foreach ($templateparams["interesse"] as $interesse) : ?>
  <?php if (!array_key_exists('id_interesse', $interesse)) : ?>
    <div class="row-justify-content-center">
      <article class="article bg-body border mb-3">
        <header class="px-3  mt-3 mb-3">
          <img src="<?php echo '<img src="' . UPLOAD_DIR . $interesse["userProfileImage"]; ?>" alt="" class="img_profile">
...
<?php foreach ($templateparams["interazione"] as $interazione) : ?>
  <?php if (!array_key_exists('id_interazione', $interazione)) : ?>
    <div class="row-justify-content-center">
      <article class="article bg-body border mb-3">
        <header class="px-3  mt-3 mb-3">
          <img src="<?php echo '<img src="' . UPLOAD_DIR . $interazione["userProfileImage"]; ?>" alt="" class="img_profile">
...
<?php foreach ($templateparams["commenti"] as $commento) : ?>
  <?php if (!array_key_exists('id_commenti', $commento)) : ?>
    <div class="row-justify-content-center">
      <article class="article bg-body border mb-3">
        <header class="px-3  mt-3 mb-3">
          <img src="<?php echo '<img src="' . UPLOAD_DIR . $commento["userProfileImage"]; ?>" alt="" class="img_profile">
