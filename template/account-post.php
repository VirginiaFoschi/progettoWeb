<?php foreach ($templateparams["annuncio"] as $annuncio): ?>
    <div class="row justify-content-center content">
        ...
        <article class="article-annuncio bg-body border mb-3">
            ...
            <footer>
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-heart-fill heart-icon <?php if(in_array($annuncio["Evento"]["id_evento"],$templateparams["likes-events"])): echo "active"; endif; ?>" viewBox="0 0 16 16" onClick="sendAjaxRequest('likes-events.php', {evento: '<?php echo $annuncio["Evento"]['id_evento']; ?>'})">
    ...
    <article class="article-recensione bg-body border mb-3">
        ...
        <footer>
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="bi bi-heart-fill heart-icon <?php 
                ...
                viewBox="0 0 16 16" onClick="sendAjaxRequest('likes-reviews.php', {autore_rec: '<?php echo $templateparams["nome-profilo"]; ?>', autore_lib: '<?php echo $recensione["Recensione"]["Autore_Libro"]; ?>', titolo: '<?php echo $recensione["Recensione"]["Titolo_Libro"]; ?>'})">
                <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314" />
            </svg>
        </footer>
    </article>
